...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
      ...
      if (match && match[1]) {
        ...
        if (predictions) {
          ...
          outputDiv.innerHTML += `
            <div class="section">
              <div class="section-title">Filter Comments by Sentiment</div>
              <div class="filter-buttons">
                <button class="filter-button all active" data-sentiment="all">All (${predictions.length})</button>
                <button class="filter-button positive" data-sentiment="1">Positive (${sentimentCounts["1"]})</button>
                <button class="filter-button neutral" data-sentiment="0">Neutral (${sentimentCounts["0"]})</button>
                <button class="filter-button negative" data-sentiment="-1">Negative (${sentimentCounts["-1"]})</button>
              </div>
            </div>`;
  
          // Add the comments section with a container for filtered comments
          outputDiv.innerHTML += `
            <div class="section">
              <div class="section-title">Comments</div>
              <div id="comments-container">
                <ul class="comment-list" id="filtered-comments">
          ...
          document.querySelectorAll('.filter-button').forEach(button => {
            button.addEventListener('click', function() {
              ...
              // Get the sentiment value to filter by
              const sentimentValue = this.getAttribute('data-sentiment');
              
              // Filter and display comments
              filterAndDisplayComments(sentimentValue);
            });
          });
        }
      } else {
    ...
    // Function to filter and display comments based on sentiment
    function filterAndDisplayComments(sentimentValue) {
      const commentsContainer = document.getElementById('filtered-comments');
      let filteredPredictions;
      
      if (sentimentValue === 'all') {
        filteredPredictions = allPredictions;
      } else {
        filteredPredictions = allPredictions.filter(item => item.sentiment === sentimentValue);
      }
      
      // Display filtered comments
      commentsContainer.innerHTML = filteredPredictions.map((item, index) => `
        <li class="comment-item">
          <span>${index + 1}. ${item.comment}</span><br>
          <span class="comment-sentiment">Sentiment: ${getSentimentLabel(item.sentiment)}</span>
        </li>`).join('');
    ...
    // Function to get a human-readable sentiment label
